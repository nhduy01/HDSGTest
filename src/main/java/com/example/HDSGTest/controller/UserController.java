...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<List<UserResponse>>> getAllUsers() {
        ...
        return ResponseEntity.ok(ApiResponse.<List<UserResponse>>builder().code(0).message("Lấy danh sách người dùng thành công").entity(users).build());
    }
    @Operation(summary = "Lấy thông tin người dùng hiện tại")
    @GetMapping()
    public ResponseEntity<ApiResponse<UserResponse>> getCurrentUserInfo() {
        UserResponse users = userService.getCurrentUserInfo();
        return ResponseEntity.ok(ApiResponse.<UserResponse>builder().code(0).message("Lấy thông tin người dùng hiện tại thành công").entity(users).build());
    }
